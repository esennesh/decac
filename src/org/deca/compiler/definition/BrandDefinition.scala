...
object EnumerationClasses {
  val apply: Memoize4[Module, String, Int, ClassBrand, ClassBrand] = Memoize4((scope: Module, name: String, tag: Int, parent: ClassBrand) => {
...
object ClassBrands {
  val apply = Memoize5((scope: Scope, name: String, parent: ClassBrand, members: List[MemberDeclaration], methods: List[MethodDeclaration]) => {
    val recordMembers: List[RecordMember] = members.map(member => RecordMember(Some(member.name), member.mutability, member.tau))
    new ClassBrand(name, new RecordType(recordMembers), methods.map(method => (method.name, method.signature.arrow.asInstanceOf[FunctionPointer])).toMap, Some(parent))
  })
}

class EnumValueDefinition(scope: Module, name: String, val tag: Int, val parent: ClassBrand) extends VariableDefinition(scope, name, new EnumerationValue(scope, name, tag, parent), new BrandType(EnumerationClasses.apply(scope, name, tag, parent), EmptyRecord), ImmutableMutability)

class EnumerationValue(scope: Module, val enum: String, val tag: Int, val parent: ClassBrand) extends ConstantExpression {
  val brand: ClassBrand = EnumerationClasses.apply(scope, enum, tag, parent)
...
class ClassDefinition(override val scope: Module,
  ...
  val brand: ClassBrand = ClassBrands.apply(scope, name, parent.brand, fields, methodDeclarations)
...
class DataConstructorDefinition(scope: Module, name: String, arguments: List[(String, MonoMutability, MonoType)], parent: ClassDefinition) extends ClassDefinition(scope, name, arguments.map(arg => (arg._1, arg._3)), Nil, parent, MemberDeclaration.dataConstructors(arguments), Nil)
